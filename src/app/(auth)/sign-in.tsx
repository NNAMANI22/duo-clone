...
// Styled with `style` + theme tokens instead of NativeWind classNames — see
// the Known Issue note in AGENTS.md and the matching workaround in
// `onboarding.tsx` / `sign-up.tsx`.
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        ...
        >
          ...
          <Text style={[typography.h1, { marginTop: 8, color: colors.foreground }]}>Welcome back</Text>
          ...
          <View style={{ marginTop: 24 }}>
            <PrimaryButton label="Log In" onPress={() => setShowVerification(true)} />
          </View>
          ...
          <View style={{ marginTop: 32, flexDirection: "row", justifyContent: "center" }}>
            <Text style={[typography.bodyMedium, { color: colors.muted }]}>Don&apos;t have an account? </Text>
            <Link href="/sign-up" replace style={[typography.bodyMedium, { color: colors.primary }]}>
              Sign up
            </Link>
          </View>
        </ScrollView>
